...
_HEADER_PANIC = '\n- PANIC ERROR '
_FOOTER_PANIC = f'{Fore.BLUE}{Style.BRIGHT}DO NOT report this error on github{Style.RESET_ALL}'
...
def catch(func):
    ...
    except PeridotPanic as e:
        size = os.get_terminal_size()
        traceback = format_exc()
        print(f'{Style.RESET_ALL}{Fore.RED}{Style.BRIGHT}{_HEADER_PANIC}{"-" * max([size[0] - len(_HEADER_PANIC) + 1, 0])}{Style.RESET_ALL}')
        ...
        for j in range(len(traceback)):
            ...
            for i in range(len(lines)):
                ...
                if ln == 'During handling of the above exception, another exception occurred:':
                    ...
                    ln = ln.split(' ')
                    ln = [ln[0][:-1], ' '.join(ln[1:])]
                    print(f'{Fore.RED}{Style.BRIGHT}{ln[0].split(".")[-1]}{Style.RESET_ALL}: {Fore.RED}{ln[1]}{Style.RESET_ALL}')
                    ...
                    print(f'    {Fore.GREEN}Line {Style.BRIGHT}{ln[1]}{Style.RESET_ALL} {Fore.GREEN}In {Style.BRIGHT}{ln[2]}{Style.RESET_ALL}')
